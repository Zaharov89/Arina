...
from Arina.typing_trainer.services import TypingTrainerValidationError, get_levels_summary, get_progress, save_animal, save_attempt
...
def normalize_layout_code() -> str:
    """Return safe layout code for public page rendering."""
    layout_code = request.args.get("layout", "ru")
    return layout_code if layout_code in LAYOUT_TITLES else "ru"


def normalize_animal_code() -> str:
    """Return safe animal code for public page rendering."""
    animal_code = request.args.get("animal", "dino")
    return animal_code if animal_code in ANIMALS else "dino"
...
def typing_trainer_levels():
    layout_code = normalize_layout_code()
    return render_template("typing_trainer/levels.html", student=get_student(), layout_code=layout_code, layout_title=LAYOUT_TITLES[layout_code], animals=ANIMALS)
...
def typing_trainer_hand_position():
    layout_code = normalize_layout_code()
    animal_code = normalize_animal_code()
...
def typing_trainer_result():
    layout_code = normalize_layout_code()
    animal_code = normalize_animal_code()
    return render_template("typing_trainer/result.html", student=get_student(), layout_code=layout_code, animal_code=animal_code, layout_title=LAYOUT_TITLES[layout_code], animal=ANIMALS.get(animal_code, ANIMALS["dino"]))
...
def api_typing_trainer_levels():
    try:
        layout_code = request.args.get("layout", "ru")
        return jsonify(get_levels_summary(get_access_token_from_request(), layout_code))
    except AuthTokenError as error:
        return jsonify({"status": "unauthorized", "message": str(error)}), 401
    except (TypingTrainerValidationError, ValueError) as error:
        return jsonify({"status": "validation_error", "message": str(error)}), 400
