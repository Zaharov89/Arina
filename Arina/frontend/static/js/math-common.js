...
function setInitialMathSetupValues() {
    ...
    if (classSelect && window.INITIAL_MATH_CLASS) {
...
function goToStep2() {
    ...
    document.getElementById('testStep4').style.display = 'none';
    const classSelect = document.getElementById('classSelect');
    if (classSelect) updateTypeOptionsForClass(classSelect.value);
}
...
function startMathTest(isSpeedMode) {
    ...
    const params = new URLSearchParams({
        class: classSelect ? classSelect.value : '1',
        type: typeSelect ? typeSelect.value : 'add_sub_to_20',
        table: tableSelect ? tableSelect.value : 'all',
        equations: includeEquations ? includeEquations.checked : false,
        parentheses: includeParentheses ? includeParentheses.checked : false,
        speed: isSpeedMode,
...
function startTestTimer() {
    ...
    testTimerInterval = setInterval(() => {
        ...
        if (timerEl) timerEl.textContent = `Время: ${minutes}:${seconds.toString().padStart(2, '0')}`;
    }, 1000);
}

function getQuestionKey(example) {
    if (!example) return '';
    if (example.question !== undefined && example.question !== null) return String(example.question).trim();
    if (example.expr !== undefined && example.expr !== null) return String(example.expr).trim();
    if (example.a !== undefined && example.op !== undefined && example.b !== undefined) return `${example.a} ${example.op} ${example.b} = ?`;
    return '';
}
...
function rememberQuestion(example) {
    ...
    if (!usedQuestionTexts.includes(questionKey)) usedQuestionTexts.push(questionKey);
    if (usedQuestionTexts.length > 300) usedQuestionTexts = usedQuestionTexts.slice(-300);
}
...
function generateQuestion() {
    ...
    .then(data => {
        ...
        if (data.question !== undefined) {
            ...
            if (Array.isArray(data.choices) && data.choices.length > 0) questionText += `\nВарианты: ${data.choices.join('   ')}`;
...
function checkAnswer() {
    ...
    if (!userAnswer) {
        ...
        emptyAnswers++;
        wrongAnswersList.push({...currentExample, example: document.getElementById('exampleDisplay')?.textContent || '', userAnswer: '(пусто)', correctAnswer: currentExample?.correct});
        disableAnswer();
        return;
    }
    ...
    if (window.testSettings?.isSpeedMode && currentSpeedTimerExpired) {
        ...
        wrongAnswers++;
        wrongAnswersList.push({...currentExample, example: document.getElementById('exampleDisplay')?.textContent || '', userAnswer: '(не успел)', correctAnswer: correct});
        disableAnswer();
        return;
    }
    ...
    .then(data => {
        if (data.result === 'correct') {
            ...
            resultMessage.className = 'result-message incorrect';
            wrongAnswers++;
            wrongAnswersList.push({...currentExample, example: document.getElementById('exampleDisplay')?.textContent || '', userAnswer: userAnswer, correctAnswer: data.correct_answer});
        }
        disableAnswer();
    })
    .catch(error => {
...
function finishTest() {
    ...
    localStorage.setItem('resultSubjectCode', 'math');
    localStorage.setItem('resultClassNumber', window.testSettings?.classNum || '1');
    localStorage.setItem('resultTopicCode', window.testSettings?.classNum === '1' ? (window.testSettings?.exampleType || '') : '');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (classSelect) {
        ...
        classSelect.addEventListener('change', function () { updateTypeOptionsForClass(this.value); });
    }
    ...
    if (answerInput) {
        answerInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                ...
                else if (nextBtn && !nextBtn.disabled) nextQuestion();
            }
        });
    }
