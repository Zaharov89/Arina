...
class MathExamplesClass2:
    ...
    def generate_example(self):
        ...
        task.setdefault('topic_title', CLASS_2_MATH_TOPICS.get(topic, {}).get('title', 'Математика 2 класс'))
        return task
    ...
    @staticmethod
    def number_task(question, correct, topic):
        return {'question': question, 'answer_type': 'number', 'correct': correct, 'topic': topic, 'topic_title': CLASS_2_MATH_TOPICS[topic].get('title')}

    @staticmethod
    def choice_task(question, choices, correct, topic):
        return {'question': question, 'answer_type': 'choice', 'choices': choices, 'correct': correct, 'topic': topic, 'topic_title': CLASS_2_MATH_TOPICS[topic].get('title')}
    ...
    def generate_add_sub_no_crossing(self):
        ...
        if random.choice([True, False]):
            return {'a': a, 'op': '+', 'b': b, 'topic': 'add_sub_no_crossing', 'topic_title': CLASS_2_MATH_TOPICS['add_sub_no_crossing']['description']}
        total = a + b
        return {'a': total, 'op': '-', 'b': b, 'topic': 'add_sub_no_crossing', 'topic_title': 'Сложение и вычитание без перехода через десяток'}
    ...
    def generate_add_sub_crossing(self):
        ...
        if self.type == '+':
            ...
            op = '-'
        elif self.type in {'all', 'addsub', 'add_sub_crossing'}:
            op = random.choice(['+', '-'])
        if op == '+':
        ...
        return {'a': a, 'op': op, 'b': b, 'topic': 'add_sub_crossing', 'topic_title': 'Сложение и вычитание с переходом через десяток'}

    def generate_addition_table(self):
        a, b = random.randint(2, 9), random.randint(2, 9)
        return {'a': a, 'op': '+', 'b': b, 'topic': 'addition_table', 'topic_title': 'Таблица сложения'}

    def generate_multiplication_table(self):
        a, b = random.randint(2, 5), random.randint(2, 10)
        return {'a': a, 'op': '*', 'b': b, 'topic': 'multiplication_table', 'topic_title': 'Таблица умножения'}

    def generate_division_equal_parts(self):
        b = random.randint(2, 5)
        q = random.randint(2, 10)
        a = b * q
        return {'a': a, 'op': '/', 'b': b, 'topic': 'division_equal_parts', 'topic_title': 'Деление на равные части'}
    ...
    def generate_word_problem(self):
        ...
        return random.choice([
            ...
            self.number_task(f'В коробке было {a} карандашей, {b} взяли. Сколько осталось?', a - b if a >= b else b - a, 'word_problems_2'),
        ])
    ...
    def generate_parentheses(self):
        ...
        expr = f'({a} + {b}) - {c}'
        return {'expr': expr, 'correct': a + b - c, 'answer_type': 'number', 'topic': 'parentheses_2', 'topic_title': 'Выражения со скобками'}
